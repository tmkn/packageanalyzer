    ...
    describe(`Error Handling`, () => {
        ...
        test(`fails on invalid lint file format`, async () => {
            const command = cli.process([
                ...
                `--package`,
                `foo`,
                path.join(`packages`, `node`, `tests`, `sampleReportMulti.js`)
            ]) as LintCommand;
        ...
        test(`fails on missing --package and --folder`, async () => {
            const command = cli.process([
                `lint`,
                path.join(`packages`, `node`, `tests`, `sampleLintFileEmpty.js`)
            ]) as LintCommand;
        ...
        test(`shows error message for invalid check return value`, async () => {
            const packageJsonPath = path.join("packages", "node", "tests", "data", "lint_data");
    ...
    describe(`--package`, () => {
        ...
        test(`correctly reports all packages in the tree`, async () => {
            const command = cli.process([
                ...
                `--package`,
                `medallo@1.0.0`,
                path.join(`packages`, `node`, `tests`, `sampleLintFileNonEmpty.js`)
            ]) as LintCommand;
