...
export class ReportService {
    private _stdoutFormatter: IFormatter;
    private _stderrFormatter: IFormatter;

    constructor(
        private readonly _config: ReportServiceConfig,
        private readonly _host: IHost
    ) {
        this._stdoutFormatter = new Formatter(this._host.getStdout());
        this._stderrFormatter = new Formatter(this._host.getStderr());
    }
    ...
    async process(): Promise<number | void> {
        ...
        try {
            ...
            await Promise.all([this._host.getStdout().flush(), this._host.getStderr().flush()]);
        }
