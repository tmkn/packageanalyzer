...
import { getPackageVersionfromString, Visitor } from "../../shared/src/visitors/visitor.js";
...
import type { Attachments } from "../../shared/src/attachments/Attachments.js";
...
describe(`Attachment Tests`, () => {
    ...
    beforeAll(() => {
        rootPath = path.join("packages", "node", "tests", "data", "testproject1");
...
describe(`Deprecated Package Tests`, () => {
    test(`Deprecation flag`, async () => {
        const rootPath = path.join("packages", "node", "tests", "data", "deprecated");
        const provider = new FileSystemPackageProvider(rootPath);
        const visitor = new Visitor(getPackageVersionFromPath(rootPath), provider, new OraLogger());
        const p = await visitor.visit();
